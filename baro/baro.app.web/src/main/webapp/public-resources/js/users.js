...
function setToFocusNumber()
{
	setToFocus("disposable_num");
}
function setToFocusCaptcha()
{
	setToFocus("captcha");
}
function setToFocusOTP()
{
	setToFocus("otp");
}
...
function unsetToFocusNumber()
{
	unsetToFocus("disposable_num");
}
function unsetToFocusForeignIP()
{
	unsetToFocus("username");
}
function unsetToFocusCaptcha()
{
	unsetToFocus("captcha");
}
function unsetToFocusOTP()
{
	unsetToFocus("otp");
}
...
function addInputEvent_Number()
{
	_addInputEvent("disposable_num", setToFocusNumber, unsetToFocusNumber);
	try{
		addInputDivEvent_Number();
	}catch (e){}
}
function addInputEvent_Abroad()
{
	_addInputEvent("username", function(){setToFocus("username")}, function(){unsetToFocus("username")});
}
function addInputEvent_AbroadContact()
{
	_addInputEvent("email", function(){setToFocus("email")}, function(){unsetToFocus("email")});
	_addInputEvent("phone_value", function(){setToFocus("phone_value")}, function(){unsetToFocus("phone_value")});
}
function addInputEvent_Captcha()
{
	_addInputEvent("captcha", setToFocusCaptcha, unsetToFocusCaptcha);
}
function addInputEvent_OTP()
{
	_addInputEvent("otp", setToFocusOTP, unsetToFocusOTP);
	try{
		addInputDivEvent_OTP();
	}catch(e){}
}
function addKeepOTPEvent()
{       
	var autoCheck = document.getElementById('sauto');
	var autoInput = document.getElementById('auto');
	autoCheck.onclick = function(event) {
...
{
	_addInputDivEvent("number_info", disp_num_info_div);
	_addInputDivEvent("number_info_div", hide_num_info_div);
}
function addInputDivEvent_Number()
{
	_addInputDivEvent("login_info", disp_login_info_div);
	_addInputDivEvent("login_info_div", hide_login_info_div);
}
function addInputDivEvent_OTP()
{
	_addInputDivEvent("otp_info", disp_otp_info_div);
	_addInputDivEvent("otp_info_div", hide_otp_info_div);
}
...
function disp_otp_info_div()
{
	clearDIValert();
	showElement('otp_info_div');
}
function hide_otp_info_div()
{
	hideElement('otp_info_div');
}
...
function disp_num_info_div()
{
	toggleElement('number_info_div');
}
function hide_num_info_div()
{
	hideElement('number_info_div');
}
...
function showAbroadErrorLayer(elementId) {
	var errLayers = ["err_name_birthday_incorrect", "err_empty_name"];

	for (var i=0,len=errLayers.length; i<len; i++) {
		var err = $(errLayers[i]);
		err.className="";
		err.style.display="none";
	}

	var showLayer = $(elementId);
	showLayer.className="error_box error_box_v";
	showLayer.style.display="block";
}
function askAbroadMoreInfo() {
	$('ask_more').value="more";
	$('frgip').submit();
}
function checkAbroadSubmit() {
	var username = $('username');
	if (username.value == "") {
		showAbroadErrorLayer('err_empty_name');
		return false;
	}
	
	return true;
}
function setContactTab(clickTab) {
	var emailTab = $('email_tab_header');
	var phoneTab = $('phone_tab_header');
	var emailBox = $('email_input_box');
	var phoneBox = $('c_phone_input_box');
	var viewTab = $('input_item');
	var email = $('email');
	var phone = $('phone_value');

	if ( clickTab == "email" ) {
		emailTab.className = "on";
		phoneTab.className = "";
...
function showAbroadContactErrorLayer(elementId) {
	var errLayers = ["err_contact_incorrect", "err_empty_email", "err_empty_phone", "err_incorrect_5times"];
...
function checkAbroadContactSubmit() {
	var selectedTab = $('input_item').value;

	if (selectedTab == "email") {
		if ($('email').value == "") {
			showAbroadContactErrorLayer('err_empty_email');
			return false;		
		}
	} else {
		if ($('phone_value').value == "") {
			showAbroadContactErrorLayer('err_empty_phone');
			return false;		
		}

		var phoneValue = $('phone_value');
		var phoneNo = $('phone_first').value + phoneValue.value;
		$('phone').value = phoneNo.replace('-', '');
	}
...
function ipCheck(a,e) {
	if (a.checked == true) {
		ipCheckOn(1);
		clickcr(a, 'log_off.iponset', '', '', e);
	} else {
		ipCheckOff();
		clickcr(a, 'log_off.ipoffset', '', '', e);
	}
}
function msieblur(a)
{
...
function getJindoSelectBoxSize(pcSize, tabletSize) {
	...
	if ( /windows ce/.test( ua ) && /polar/.test( ua ) ) {
		...
		return tabletSize;
	} else if ( /iphone/.test( ua ) || /ipod/.test( ua ) || /ipad/.test( ua ) ) {
		return tabletSize;
	} else if ( /android/.test( ua ) ) {
		return tabletSize;
	} else if ( /dolfin/.test( ua )) {
		return tabletSize;
		...
		return tabletSize;
	} else if ( /windows phone os/.test( ua ) && /iemobile/.test( ua )){
		return tabletSize;
	} else {
		return pcSize;
	}
...
function reCaptcha() {
        $('chptchaimg').src=$('chptchaimg').src+'1';
		$('captcha').focus();
}
function getCaptchaVal() {
        return $('chptcha').value;
}
function getCaptchaKey() {
        return $('chptchakey').value;
}
...
function changeCaptchaMode() {
	...
	var imageDiv = $('image_captcha_div');
	var soundDiv = $('sound_captcha_div');
	toggle(imageDiv);
	toggle(soundDiv);
...
function confirm_submit()
{
	if ($('id').value == "") {
		clearDIValert();
		showElement("div_noid");
		$('id').focus();
		return false;
	} else if ($('pw').value == "") {
		clearDIValert();
		showElement("div_nopw");
		$('pw').focus();
		return false;
	}
	return encrypt_idpw();
...
function confirm_number_submit()
{
	if ($('disposable_num').value == "") {
		clearDIValert();
		showElement("div_no_disposable_num");
		return false;
	}
	try {
	...
	{
		clearDIValert();
		showElement("div_no_disposable_only_num");
		$('disposable_num').value = ""
		return false;
	}
	return true;
}
function confirm_captcha_submit()
{
	var handleError = function(input, errorMessage) {
		alert(errorMessage);
		input.value = "";
		input.focus();
		return false;
	...
	var id = $('id');
	if(id.value == "") {
		return handleError(id, idNoInputError);
	}

	var pw = $('pw');
	if(pw.value == "") {
		return handleError(pw, pwNoInputError);
	}

	var captcha = $('captcha');
	if(captcha.value == "") {
		return handleError(pw, captchaNoInputError);
	}
...
function confirm_otp_submit()
{
	var otp = $('otp');
	var showErrorLayer = function(noInputDisplay, incorrectDisplay)
	{
		clearDIValert();
		$('otp_error_noinput').style.display = noInputDisplay;
		$('otp_error_incorrect').style.display = incorrectDisplay;
	}
	...
	if (!isNumeric(otp.value))
	{
		showErrorLayer("none", "block");
		clearOTP();
		return false;
	}
